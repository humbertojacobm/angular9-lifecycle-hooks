...
export class ChildComponent implements AfterContentChecked, AfterContentInit {
  ...
  constructor(private logger: LoggerService) {
    this.logIt('ChildComponent constructor');
  }

  private logIt(method: string) {
    //let child = this.contentChild;
    let message = `${method}: child content`;
    this.logger.log(message);
  }

  ngAfterContentInit() {
    // contentChild is set after the content has been initialized
    this.logIt('ChildComponent AfterContentInit');
  ...
  ngAfterContentChecked() {
    // contentChild is updated after the content has been checked
      ...
      this.logIt(' ChildComponent AfterContentChecked (no change)');
